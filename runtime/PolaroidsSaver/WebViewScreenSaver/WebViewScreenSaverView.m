...
- (void)startAnimation {
  ...
  // === BEGIN PHASE 0 SMOKE TEST MODIFICATION ===
  // Load the bundled smoke-test HTML directly from Resources/web/index.html.
  // Bypasses liquidx's user-pref URL list (WVSSAddress/WVSSConfig) entirely —
  // Phase 0 is a fixed smoke test, not a configurable webview screensaver.
  NSBundle *bundle = [NSBundle bundleForClass:[self class]];
  NSURL *htmlURL = [bundle URLForResource:@"index"
                            withExtension:@"html"
                             subdirectory:@"web"];
  if (htmlURL) {
    NSURL *webDir = [htmlURL URLByDeletingLastPathComponent];
    [_webView loadFileURL:htmlURL allowingReadAccessToURL:webDir];
  } else {
    WVSSLog(@"FATAL: smoke test index.html not found in bundle Resources/web/");
  }
  // === END PHASE 0 SMOKE TEST MODIFICATION ===
...
- (void)stopAnimation {
  ...
  [self teardown];
  // === PHASE 0 GATE-02 BELT-AND-SUSPENDERS ===
  // On Sequoia 15.7+, the `com.apple.screensaver.willstop` distributed
  // notification does NOT always fire when the screensaver is dismissed
  // (verified empirically: PID lingered after dismiss). stopAnimation IS
  // guaranteed by Apple's ScreenSaverView framework contract to be called
  // on real dismissal, so we hook here as well.
...
- (void)screensaverWillStop:(NSNotification *)notification {
  ...
  if (@available(macOS 14.0, *)) {
    [self teardown];
    // === PHASE 0 GATE-02 MODIFICATION ===
    // Force-terminate the legacyScreenSaver host process. Without this, on
    // Sonoma+/Sequoia 15 the host can linger eating CPU/memory after dismissal.
    // Aerial reference pattern: github.com/JohnCoates/Aerial commit 8c78e7c.
    // No isPreview guard needed: startAnimation early-returns in preview mode,
    // so the WebView (and notification subscription) never exist in preview.
    exit(0);
  }
}
